...
param (
    ...
    [Parameter(Mandatory = $true, Position = 3, ValueFromPipelineByPropertyName = $true,
        ValueFromPipeline = $True,
        HelpMessage = 'Please specify ExtensionAttribute number'
    )]
...
function Set-AzureAdDeviceExtensionAttribute {
    ...
    # set common endpoint
    #$authorityUri = "https://login.microsoftonline.com/common/oauth2/v2.0/authorize"
    #Write-Verbose $('{0} - Using {1} auth endpoint' -f $(Get-Date -Format T), $authorityUri)
    #Write-Verbose $('{0} - Using {1} resource endpoint' -f $(Get-Date -Format T), $resourceUri)
    
    # Create AuthenticationContext for acquiring token
    #add-type -Path "C:\Program Files\WindowsPowerShell\Modules\AzureADPreview\2.0.2.129\Microsoft.IdentityModel.Clients.ActiveDirectory.dll"
    #Add-Type -Path "C:\Program Files\WindowsPowerShell\Modules\AzureADPreview\2.0.2.129\Microsoft.IdentityModel.Clients.ActiveDirectory.Platform.dll"
    
    #Add-Type -Path "C:\Program Files\WindowsPowerShell\Modules\AzureAD\2.0.2.31\Microsoft.IdentityModel.Clients.ActiveDirectory.dll"        
    #Add-Type -Path "C:\Program Files\WindowsPowerShell\Modules\AzureAD\2.0.2.31\Microsoft.IdentityModel.Clients.ActiveDirectory.Platform.dll"     
    #$authContext = [Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationContext]::new($authorityUri, $false) 
    
    # set clientid and redirect URI for Azure PowerShell
    #$clientId = "1950a258-227b-4e31-a9cf-717495945fc2" 
    #$redirectUri = "urn:ietf:wg:oauth:2.0:oob"
    
    # Acquire & return the authentication token
    #try {
    #    $platformParameters = [Microsoft.IdentityModel.Clients.ActiveDirectory.PlatformParameters]::new([Microsoft.IdentityModel.Clients.ActiveDirectory.PromptBehavior]::Auto)
    #    $accessToken = $authContext.AcquireTokenAsync($resourceUri, $clientId, $redirectUri, $platformParameters).Result.AccessToken        
    #}
    #catch {
    #    Write-Error $('Could not obtain an access token - {0}' -f $Error[0].Exception)
    #}
    ...
    #$apiUri = "https://graph.microsoft.com/v1.0/devices/{0}" -f $ObjectId
    $Uri = "https://graph.microsoft.com/$graphApiVersion/$graphResource/{0}" -f $ObjectId  
    $apiBodyJSON = [PSCustomObject]@{
        "extensionAttributes" = [PSCustomObject]@{
            $ExtensionAttribute = $Value 
        } 
    } | ConvertTo-Json

    Write-Verbose $('{0} - Using {1} API URI' -f $(Get-Date -Format T), $apiUri)

    #patch API
    try {
        $restMethod = Invoke-RestMethod -Uri $Uri -Headers $authToken -ContentType "application/json" -Method Patch -Body $apiBodyJSON
        Write-Verbose $('{0} - Updated DeviceId {1}: {2}={3}' -f $(Get-Date -Format T), $ObjectId, $ExtensionAttribute, $Value)
        
    }
    catch {
        Write-Error $('Could not update device data at {0} - {1}' -f $apiUri, $Error[0].Exception)
    }
...
Function Get-DeviceID {
    ...
    Process {
        ...
        Else {
            Write-Log -Message "Found device: `n$deviceID"
        }
...
foreach ($device in $DeviceName) {
    ...
    Write-Log -Message "Setting ObjectId attributes..."
    Set-AzureAdDeviceExtensionAttribute -ObjectId $($objDevice.ObjectId) -ExtensionAttribute $ExtensionAttributeNumber -Value $AttributeValue
    Write-Log -Message "Updated $device $ExtensionAttributeNumber to $AttributeValue"
}
